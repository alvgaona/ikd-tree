...
constexpr int Minimal_Unbalanced_Tree_Size = 10;
constexpr int Multi_Thread_Rebuild_Point_Num = 1500;
constexpr bool DOWNSAMPLE_SWITCH = true;
} // namespace

namespace ikd_tree {

// Full definitions of the types forward-declared in the header. Kept here so
// pthread.h stays out of the public surface.
template <typename PointType> struct KdTree<PointType>::Node {
    PointType point;
...
template <typename PointType> struct KdTree<PointType>::Threads {
    pthread_t rebuild_thread;
    pthread_mutex_t termination_flag_mutex_lock;
    pthread_mutex_t rebuild_ptr_mutex_lock;
    pthread_mutex_t working_flag_mutex;
    pthread_mutex_t search_flag_mutex;
    pthread_mutex_t rebuild_logger_mutex_lock;
    pthread_mutex_t points_deleted_rebuild_mutex_lock;
};

template <typename PointType>
KdTree<PointType>::KdTree(float delete_param, float balance_param, float box_length)
...
template <typename PointType> void KdTree<PointType>::set_delete_criterion_param(float delete_param) {
    delete_criterion_param = delete_param;
}

template <typename PointType> void KdTree<PointType>::set_balance_criterion_param(float balance_param) {
    balance_criterion_param = balance_param;
}

template <typename PointType> void KdTree<PointType>::set_downsample_param(float downsample_param) {
    downsample_size = downsample_param;
}

template <typename PointType>
void KdTree<PointType>::initialize(float delete_param, float balance_param, float box_length) {
    set_delete_criterion_param(delete_param);
    set_balance_criterion_param(balance_param);
    set_downsample_param(box_length);
}

template <typename PointType> void KdTree<PointType>::InitTreeNode(Node *root) {
    root->point.x = 0.0f;
    root->point.y = 0.0f;
    root->point.z = 0.0f;
...
template <typename PointType> int KdTree<PointType>::size() {
    int s = 0;
    if (Rebuild_Ptr == nullptr || *Rebuild_Ptr != Root_Node) {
        ...
        if (!pthread_mutex_trylock(&threads_->working_flag_mutex)) {
            s = Root_Node->TreeSize;
            pthread_mutex_unlock(&threads_->working_flag_mutex);
            return s;
        } else {
            return Treesize_tmp;
        }
...
template <typename PointType> BoxPointType KdTree<PointType>::tree_range() {
    BoxPointType range;
    if (Rebuild_Ptr == nullptr || *Rebuild_Ptr != Root_Node) {
        if (Root_Node != nullptr) {
...
template <typename PointType> int KdTree<PointType>::validnum() {
    int s = 0;
    if (Rebuild_Ptr == nullptr || *Rebuild_Ptr != Root_Node) {
        if (Root_Node != nullptr)
...
template <typename PointType> void KdTree<PointType>::root_alpha(float &alpha_bal, float &alpha_del) {
    if (Rebuild_Ptr == nullptr || *Rebuild_Ptr != Root_Node) {
...
template <typename PointType> void KdTree<PointType>::start_thread() {
    pthread_mutex_init(&threads_->termination_flag_mutex_lock, NULL);
    pthread_mutex_init(&threads_->rebuild_ptr_mutex_lock, NULL);
    pthread_mutex_init(&threads_->rebuild_logger_mutex_lock, NULL);
    pthread_mutex_init(&threads_->points_deleted_rebuild_mutex_lock, NULL);
    pthread_mutex_init(&threads_->working_flag_mutex, NULL);
    pthread_mutex_init(&threads_->search_flag_mutex, NULL);
    pthread_create(&threads_->rebuild_thread, NULL, multi_thread_ptr, (void *) this);
}

template <typename PointType> void KdTree<PointType>::stop_thread() {
    pthread_mutex_lock(&threads_->termination_flag_mutex_lock);
    termination_flag = true;
    pthread_mutex_unlock(&threads_->termination_flag_mutex_lock);
    if (threads_->rebuild_thread)
        pthread_join(threads_->rebuild_thread, NULL);
    pthread_mutex_destroy(&threads_->termination_flag_mutex_lock);
    pthread_mutex_destroy(&threads_->rebuild_logger_mutex_lock);
    pthread_mutex_destroy(&threads_->rebuild_ptr_mutex_lock);
    pthread_mutex_destroy(&threads_->points_deleted_rebuild_mutex_lock);
    pthread_mutex_destroy(&threads_->working_flag_mutex);
    pthread_mutex_destroy(&threads_->search_flag_mutex);
}

template <typename PointType> void *KdTree<PointType>::multi_thread_ptr(void *arg) {
    KdTree *handle = (KdTree *) arg;
    handle->multi_thread_rebuild();
    return nullptr;
}

template <typename PointType> void KdTree<PointType>::multi_thread_rebuild() {
    bool terminated = false;
    Node *father_ptr, **new_node_ptr;
    pthread_mutex_lock(&threads_->termination_flag_mutex_lock);
    terminated = termination_flag;
    pthread_mutex_unlock(&threads_->termination_flag_mutex_lock);
    while (!terminated) {
        pthread_mutex_lock(&threads_->rebuild_ptr_mutex_lock);
        pthread_mutex_lock(&threads_->working_flag_mutex);
        if (Rebuild_Ptr != nullptr) {
            ...
            // Lock Search
            pthread_mutex_lock(&threads_->search_flag_mutex);
            while (search_mutex_counter != 0) {
                pthread_mutex_unlock(&threads_->search_flag_mutex);
                usleep(1);
                pthread_mutex_lock(&threads_->search_flag_mutex);
            }
            ...
            pthread_mutex_lock(&threads_->points_deleted_rebuild_mutex_lock);
            flatten(*Rebuild_Ptr, Rebuild_PCL_Storage, MULTI_THREAD_REC);
            // Unlock deleted points cache
            pthread_mutex_unlock(&threads_->points_deleted_rebuild_mutex_lock);
            // Unlock Search
            pthread_mutex_lock(&threads_->search_flag_mutex);
            ...
            if (int(Rebuild_PCL_Storage.size()) > 0) {
                ...
                pthread_mutex_lock(&threads_->working_flag_mutex);
                pthread_mutex_lock(&threads_->rebuild_logger_mutex_lock);
                int tmp_counter = 0;
                while (!Rebuild_Logger.empty()) {
                    Operation = Rebuild_Logger.front();
                    max_queue_size = max(max_queue_size, static_cast<int>(Rebuild_Logger.size()));
                    Rebuild_Logger.pop_front();
                    pthread_mutex_unlock(&threads_->rebuild_logger_mutex_lock);
                    pthread_mutex_unlock(&threads_->working_flag_mutex);
        ...
        pthread_mutex_unlock(&threads_->rebuild_ptr_mutex_lock);
        pthread_mutex_lock(&threads_->termination_flag_mutex_lock);
        terminated = termination_flag;
        pthread_mutex_unlock(&threads_->termination_flag_mutex_lock);
        usleep(100);
...
template <typename PointType> void KdTree<PointType>::run_operation(Node **root, const OperationLog &operation) {
    switch (operation.op) {
    case ADD_POINT:
...
template <typename PointType> void KdTree<PointType>::build(PointVector point_cloud) {
    if (Root_Node != nullptr) {
        delete_tree_nodes(&Root_Node);
    }
    if (STATIC_ROOT_NODE != nullptr) {
...
template <typename PointType>
void KdTree<PointType>::nearest_search(const PointType &point, int k_nearest, PointVector &Nearest_Points,
    ...
    for (int i = 0; i < k_found; i++) {
        Nearest_Points.push_back(q.top().point);
        Point_Distance.push_back(q.top().dist);
        q.pop();
    }
    reverse(Nearest_Points.begin(), Nearest_Points.end());
    reverse(Point_Distance.begin(), Point_Distance.end());
    return;
}

template <typename PointType>
void KdTree<PointType>::box_search(const BoxPointType &Box_of_Point, PointVector &Storage) {
    Storage.clear();
    Search_by_range(Root_Node, Box_of_Point, Storage);
}

template <typename PointType>
void KdTree<PointType>::radius_search(const PointType &point, const float radius, PointVector &Storage) {
    Storage.clear();
    Search_by_radius(Root_Node, point, radius, Storage);
}

template <typename PointType> int KdTree<PointType>::add_points(PointVector &PointToAdd, bool downsample_on) {
    ...
    for (int i = 0; i < PointToAdd.size(); i++) {
        if (downsample_switch) {
            ...
            if (Rebuild_Ptr == nullptr || *Rebuild_Ptr != Root_Node) {
                ...
                if (Downsample_Storage.size() > 1 || same_point(PointToAdd[i], downsample_result)) {
                    OperationLog operation_delete, operation;
...
template <typename PointType> void KdTree<PointType>::add_point_boxes(vector<BoxPointType> &BoxPoints) {
...
template <typename PointType> int KdTree<PointType>::delete_points(PointVector &PointToDel) {
    int before = validnum();
...
template <typename PointType> int KdTree<PointType>::delete_point_boxes(vector<BoxPointType> &BoxPoints) {
    int tmp_counter = 0;
    for (int i = 0; i < BoxPoints.size(); i++) {
        if (Rebuild_Ptr == nullptr || *Rebuild_Ptr != Root_Node) {
...
template <typename PointType> void KdTree<PointType>::acquire_removed_points(PointVector &removed_points) {
    pthread_mutex_lock(&threads_->points_deleted_rebuild_mutex_lock);
...
template <typename PointType> void KdTree<PointType>::BuildTree(Node **root, int l, int r, PointVector &Storage) {
    if (l > r)
        return;
    *root = new Node;
    InitTreeNode(*root);
...
template <typename PointType> void KdTree<PointType>::Rebuild(Node **root) {
    Node *father_ptr;
    if ((*root)->TreeSize >= Multi_Thread_Rebuild_Point_Num) {
        if (!pthread_mutex_trylock(&threads_->rebuild_ptr_mutex_lock)) {
...
template <typename PointType>
int KdTree<PointType>::Delete_by_range(Node **root, BoxPointType boxpoint, bool allow_rebuild, bool is_downsample) {
...
template <typename PointType>
void KdTree<PointType>::Delete_by_point(Node **root, const PointType &point, bool allow_rebuild) {
    if ((*root) == nullptr || (*root)->tree_deleted)
        return;
    (*root)->working_flag = true;
    Push_Down(*root);
    ...
    OperationLog delete_log;
    struct timespec Timeout;
    delete_log.op = DELETE_POINT;
    delete_log.point = point;
    // nth_element may place points equal to the pivot in either subtree,
    // so recurse into both whenever the coordinate matches the pivot.
    float p_axis = (*root)->division_axis == 0 ? point.x : ((*root)->division_axis == 1 ? point.y : point.z);
    float r_axis = (*root)->division_axis == 0 ? (*root)->point.x
                                               : ((*root)->division_axis == 1 ? (*root)->point.y : (*root)->point.z);
    bool go_left = p_axis <= r_axis;
    bool go_right = p_axis >= r_axis;
    if (go_left) {
        if ((Rebuild_Ptr == nullptr) || (*root)->left_son_ptr != *Rebuild_Ptr) {
...
template <typename PointType>
void KdTree<PointType>::Add_by_range(Node **root, BoxPointType boxpoint, bool allow_rebuild) {
    if ((*root) == nullptr)
        return;
    (*root)->working_flag = true;
    Push_Down(*root);
...
template <typename PointType>
void KdTree<PointType>::Add_by_point(Node **root, const PointType &point, bool allow_rebuild, int father_axis) {
    if (*root == nullptr) {
        *root = new Node;
        InitTreeNode(*root);
        (*root)->point = point;
...
template <typename PointType>
void KdTree<PointType>::Search(Node *root, int k_nearest, const PointType &point, Heap &q, double max_dist) {
    if (root == nullptr || root->tree_deleted)
        return;
    ...
    if (!root->point_deleted) {
        ...
        if (dist <= max_dist_sqr && (q.size() < k_nearest || dist < q.top().dist)) {
            if (q.size() >= k_nearest)
                q.pop();
            PointCmp current_point{root->point, dist};
...
template <typename PointType>
void KdTree<PointType>::Search_by_range(Node *root, BoxPointType boxpoint, PointVector &Storage) {
    if (root == nullptr)
        return;
    Push_Down(root);
    if (boxpoint.vertex_max[0] <= root->node_range_x[0] || boxpoint.vertex_min[0] > root->node_range_x[1])
...
template <typename PointType>
void KdTree<PointType>::Search_by_radius(Node *root, const PointType &point, float radius, PointVector &Storage) {
    if (root == nullptr)
        return;
    Push_Down(root);
    ...
    float node_radius = sqrt(root->radius_sq);
    if (dist > radius + node_radius)
        return;
    if (dist <= radius - node_radius) {
        flatten(root, Storage, NOT_RECORD);
        return;
...
template <typename PointType> bool KdTree<PointType>::Criterion_Check(Node *root) {
    if (root->TreeSize <= Minimal_Unbalanced_Tree_Size) {
        return false;
    }
    float balance_evaluation = 0.0f;
    float delete_evaluation = 0.0f;
    Node *son_ptr = root->left_son_ptr;
    if (son_ptr == nullptr)
...
template <typename PointType> void KdTree<PointType>::Push_Down(Node *root) {
    if (root == nullptr)
        return;
    OperationLog operation;
...
template <typename PointType> void KdTree<PointType>::Update(Node *root) {
    Node *left_son_ptr = root->left_son_ptr;
    Node *right_son_ptr = root->right_son_ptr;
    ...
    if (left_son_ptr != nullptr && right_son_ptr != nullptr) {
        ...
        // Always include all subtree extents (alive or deleted) so Add_by_range
        // can still reach soft-deleted regions for revival. Search pruning
        // pays a slightly looser bound but stays correct.
    ...
    root->node_range_x[0] = tmp_range_x[0];
    root->node_range_x[1] = tmp_range_x[1];
    root->node_range_y[0] = tmp_range_y[0];
    root->node_range_y[1] = tmp_range_y[1];
    root->node_range_z[0] = tmp_range_z[0];
    root->node_range_z[1] = tmp_range_z[1];
...
template <typename PointType>
void KdTree<PointType>::flatten(Node *root, PointVector &Storage, delete_point_storage_set storage_type) {
    if (root == nullptr)
        return;
    Push_Down(root);
    if (!root->point_deleted) {
...
template <typename PointType> void KdTree<PointType>::flatten(PointVector &Storage) {
    Storage.clear();
    flatten(Root_Node, Storage, NOT_RECORD);
}

template <typename PointType> void KdTree<PointType>::delete_tree_nodes(Node **root) {
    if (*root == nullptr)
        return;
    Push_Down(*root);
...
template <typename PointType> bool KdTree<PointType>::same_point(const PointType &a, const PointType &b) {
    return (fabs(a.x - b.x) < EPSS && fabs(a.y - b.y) < EPSS && fabs(a.z - b.z) < EPSS);
}

template <typename PointType> float KdTree<PointType>::calc_dist(const PointType &a, const PointType &b) {
    float dist = 0.0f;
    dist = (a.x - b.x) * (a.x - b.x) + (a.y - b.y) * (a.y - b.y) + (a.z - b.z) * (a.z - b.z);
    return dist;
}

template <typename PointType> float KdTree<PointType>::calc_box_dist(Node *node, const PointType &point) {
    if (node == nullptr)
        return INFINITY;
    float min_dist = 0.0;
...
template <typename PointType> bool KdTree<PointType>::point_cmp_x(PointType a, PointType b) {
    return a.x < b.x;
}
template <typename PointType> bool KdTree<PointType>::point_cmp_y(PointType a, PointType b) {
    return a.y < b.y;
}
template <typename PointType> bool KdTree<PointType>::point_cmp_z(PointType a, PointType b) {
    return a.z < b.z;
}

template class KdTree<ikdTree_PointType>;
